...
public class AirplaneTicketController {
    ...
    public AirplaneTicket getTicketDetails(final String ticketId) {
        boolean ok=tickets.stream().filter(d->(d.getId().compareTo(ticketId))==0).findAny().isPresent();
        if(ok==true)
        {
            return tickets.stream().filter(d->(d.getId().compareTo(ticketId))==0).findAny().get();
        }
        else
        {
     ...
     */
    public void buyTicket(final String destination, final String customerId) throws RuntimeException {
        List<AirplaneTicket> list=tickets.stream().filter(d->(d.getDestination().compareTo(destination))==0).collect(Collectors.toList());
        if(list.isEmpty())
        {
        ...
        {
            if(list.stream().filter(d->(d.getStatus().compareTo(TicketStatus.NEW))==0).findFirst().isEmpty())
            {
                throw new NoTicketAvailableException();
            }
            else
            {
                list.stream().filter(d->(d.getStatus().compareTo(TicketStatus.NEW))==0).findFirst().get().setCustomerId(customerId);
                list.stream().filter(d->(d.getStatus().compareTo(TicketStatus.NEW))==0).findFirst().get().setStatus(TicketStatus.ACTIVE);
            }
        }
    }
    ...
    public void cancelTicket(final String ticketId) {
        ...
        if(ok==true)
        {
            if(tickets.stream().filter(d->(d.getId().compareTo(ticketId))==0).findAny().get().getCustomerId().compareTo(null)==0)
            {
                throw new TicketNotAssignedException();
            }
            else
            {
                tickets.stream().filter(d->(d.getId().compareTo(ticketId))==0).findAny().get().setStatus(TicketStatus.CANCELED);
            }
        }
        else
        {
    ...
    public void changeTicketCustomerId(final String ticketId, final String customerId) {
        ...
        if(ok==true)
        {
            ...
            {
                tickets.stream().filter(d->(d.getId().compareTo(ticketId))==0).findAny().get().setCustomerId(customerId);
            }
        }
        else
        {
    ...
    public List<AirplaneTicket> filterTicketsByStatus(final TicketStatus status) {
        return tickets.stream().filter(d->(d.getStatus().compareTo(status))==0).collect(Collectors.toList());
    }
    ...
    public Map<String, List<AirplaneTicket>> groupTicketsByCustomerId() {
        Map<String,List<AirplaneTicket>> groupby=new HashMap<>();
        for(AirplaneTicket ticket:tickets)
        {
            String c_id=ticket.getCustomerId();
            if(groupby.containsKey(c_id))
            {
                groupby.get(c_id).add(ticket);
            }
            else
            {
               List<AirplaneTicket> customer_nou=new ArrayList<>();
               customer_nou.add(ticket);
                groupby.put(c_id,customer_nou);
            }
        }
        return groupby;
    }
